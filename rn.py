    ...
    def __init__(self, anchor, dropout=0.1):
        ...
        if anchor == 49:
            self.rn = nn.Sequential(
                ...
                nn.Dropout(dropout),
                nn.Linear(343, 48, bias=True),
                nn.ReLU(inplace=True),
                nn.Dropout(dropout),
                nn.Linear(48, 16, bias=True)
            )
        ...
        # 0.9
            ...
            self.rn = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Dropout(dropout),
                nn.Linear(1728, 108, bias=True),
                nn.ReLU(inplace=True),
                nn.Dropout(dropout),
                nn.Linear(108, 16, bias=True)
            )
...
def Relation(x):
    ...
    mean_values = torch.mean(max_values, dim=1, keepdim=True)
    x =  torch.sigmoid(mean_values)
    return x