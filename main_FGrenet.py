...
from utils_FG import DomainDataset, compute_metric, parse_args
...
def train(net, data_loader, train_optimizer):
    ...
    for img, neg_img,pos, neg, label,skneg_label,imneg_label in train_bar:
        ...
        neg_emb = net(neg.cuda(), img_type='photo') # photo
        # triplet_loss
        ...
        cls_loss = (cls_sketch_loss+cls_photo_loss)
        # L_SCL
        ...
        sk1 = logit_scale * sk @ text_emb.t()
        im1 = logit_scale * im @ text_emb.t()
        sk, im,sk1,im1 = sk.cuda(), im.cuda(),sk1.cuda(),im1.cuda()
        ...
        cos_scores1 = cos_similar(sk1, im1)
        rn_scores = torch.sigmoid(cos_scores)
        rn_scores1 = torch.sigmoid(cos_scores1)
        lossrn = rn_loss(rn_scores, c.cuda())   # The initial value of lossrn should be around 1.00.
        lossrn1 = rn_loss(rn_scores1, c.cuda())
        lossrn = float(lossrn+lossrn1)
        ###############################
        loss = triplet_loss*4 +(cls_loss) *0.5 +L_SCL*15+lossrn*4
        # loss = triplet_loss*4  + (cls_loss) * 0.5+L_SCL*15+lossrn*4
        ...
        train_bar.set_description('Train Epoch: [{}/{}] Loss: {:.4f} triplet_loss:{:.4f} lossrn:{:.4f}'
                                  .format(epoch, args.epochs, total_loss / total_num,triplet_loss / total_num,lossrn/ total_num))
...
def val(net, data_loader):
    ...
    with torch.no_grad():
        ...
        print(acc)
        results['acc@1'].append(acc['acc@1'] * 100)
        results['acc@5'].append(acc['acc@5'] * 100)
        results['acc@10'].append(acc['acc@10'] * 100)
        print('Val Epoch: [{}/{}] | P@1:{:.1f}% | P@5:{:.1f}% | P@10:{:.1f}% '
              .format(epoch, args.epochs, acc['acc@1'] * 100, acc['acc@5'] * 100, acc['acc@10'] * 100))
...
if __name__ == '__main__':
    ...
    args = parse_args()
    t = ''#
    num_class = 300 # ChairV2=300 shoe=1800
    ...
    if args.mode == 'train':
        ...
        # optimizer = Adam([{'params': model.sketch_encoder.parameters(), 'lr': args.encoder_lr},
        #                   {'params': [model.sketch_prompt], 'lr': args.prompt_lr}])
        # training loop
        results = {'train_loss': [], 'val_precise': [], 'acc@1': [], 'acc@5': [], 'acc@10': []}
        best_precise = 0.0
