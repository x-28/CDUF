...
class DomainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            other_classes = sorted(set(self.classes.keys()) - {self.names[label]})
            other_dirname = os.path.basename(os.path.normpath(np.random.choice(other_classes)))
            neg_name1 = np.random.choice(self.refs[other_dirname])
            neg_img = self.transform(Image.open(neg_name1))
            # pos_name = np.random.choice(self.refs[self.names[label]])  源代码
...
def compute_metric(vectors, domains, labels):
    ...
    acc['precise'] = (acc['acc@1'] + acc['acc@5'] + acc['acc@10']) / 3 #细粒度
    # the mean value is chosen as the representative of precise
    #acc['precise'] = (acc['P@100'] + acc['P@200'] + acc['mAP@200'] + acc['mAP@all']) / 4 #粗粒度
    return acc
...
def parse_args():
    ...
    parser.add_argument('--data_name', default='ChairV2', type=str, choices=['sketchy', 'tuberlin','QuickDraw','ShoeV2','ChairV2'],
                        help='Dataset name')
